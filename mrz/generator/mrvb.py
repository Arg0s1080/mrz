...
class MRVBCodeGenerator(TD2CodeGenerator):
    """Calculate the string code for machine readable zone visas of type B (MRVB)
    ...
    def __init__(self,
                 ...
                 force=False):
        TD2CodeGenerator.__init__(self, document_type, country_code, surname, given_names, document_number,
                                  nationality, birth_date, sex, expiry_date, optional_data, transliteration,
                                  force)

    @property
    def optional_data(self) -> str:
        """Return optional data field for Visas of type B (MRVB)

        Optional data at the discretion of the issuing State or organization. Non mandatory field.
        29 to 36 char position of the second line.
    ...
    def optional_data(self, value: str):
        """Set optional data field for Visas of type B (MRVB)
        ...
        """
        self._optional_data = check.field(transliterate(value, self.transliteration), 8, "id number", "<")

    @property
    def final_hash(self) -> None:
        # override property (TD2)
        return None
